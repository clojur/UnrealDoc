...
public:
	 ...
	 * be changed.
	 */
	FDpUniformTessellate(FDynamicMesh3* OutMesh) 
	:
	 ...
	 */
	FDpUniformTessellate(const FDynamicMesh3* Mesh, FDynamicMesh3* OutMesh) 
	: 
	ResultMesh(OutMesh), Mesh(Mesh), bInPlace(false)
	{
	}
	
	virtual ~FDpUniformTessellate() 
	{
	}
